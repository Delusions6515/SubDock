...
case "$target" in
  ...
  darwin-arm64) mihomo_asset="mihomo-darwin-arm64-v1"; mihomo_archive=gz ;;
...
if test "$mihomo_archive" = gz; then gzip -dc "$work_dir/$mihomo_file" > "$staging/mihomo"; else unzip -p "$work_dir/$mihomo_file" '*/mihomo.exe' > "$staging/mihomo.exe"; fi
